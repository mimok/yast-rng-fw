...
USB_DMA_INIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE)
static uint8_t s_countryCode[COMM_FEATURE_DATA_SIZE] = { (COUNTRY_SETTING >> 0U)
		& 0x00FFU, (COUNTRY_SETTING >> 8U) & 0x00FFU };
...
#ifndef RNGSPEED
#define RNGSPEED 0 //Fast = 0, Standard = 1, Secure = 3
#endif

#if (RNGSPEED&0x02) == 0
#define PPREDICTION_RESISTANCE MBEDTLS_CTR_DRBG_PR_OFF
#else
#define PPREDICTION_RESISTANCE MBEDTLS_CTR_DRBG_PR_ON
#endif

#if (RNGSPEED&0x01) == 0
#define DRBG_DISABLED 0
#else
#define DRBG_DISABLED 1
#endif


static int entropy_source(void *data, uint8_t *output, size_t len) {
	se_get_random(output, len);
	return 0;
}
...
void APPInit(void) {
	...
	mbedtls_ctr_drbg_seed(&ctr_drbg, entropy_source, &entropy, NULL, 0);
	mbedtls_ctr_drbg_set_prediction_resistance(&ctr_drbg, PPREDICTION_RESISTANCE);
...
const static uint8_t FULL_FLAG = 0xA5;
const static uint8_t EMPTY_FLAG = 0x5A;
const static uint32_t RANDLEN = 1024;

void APPTask(void) {
	static uint8_t bufferState = EMPTY_FLAG;
	usb_status_t error = kStatus_USB_Error;

	//Pre-compute random data
	if (bufferState == EMPTY_FLAG) {
#if DRBG_DISABLED
		se_get_random(s_currSendBuf, RANDLEN);
#else
		mbedtls_ctr_drbg_random(&ctr_drbg, s_currSendBuf, RANDLEN);
#endif
		bufferState = FULL_FLAG;
	}

	if ((1 == s_cdcVcom.attach) && (1 == s_cdcVcom.startTransactions) && (bufferState == FULL_FLAG)) {
		/* User Code */
		/* endpoint callback length is USB_CANCELLED_TRANSFER_LENGTH (0xFFFFFFFFU) when transfer is canceled */
		if ((0 != s_recvSize)
				&& (USB_CANCELLED_TRANSFER_LENGTH != s_recvSize)) {
			s_recvSize = 0;
		}
		...
		error = USB_DeviceCdcAcmSend(s_cdcVcom.cdcAcmHandle,
				USB_CDC_VCOM_BULK_IN_ENDPOINT, s_currSendBuf, RANDLEN);

		if (error != kStatus_USB_Success) {
			ERROR_TRAP();
		} else {
			bufferState = EMPTY_FLAG;
		}
